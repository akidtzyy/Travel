...
// Service role client — bypasses RLS, only used on server
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (requester_id === target_user_id) {
      return res.status(403).json({ error: 'You cannot change your own role (self-demotion prevention)' });
    }

    const adminClient = getAdminClient();

    // ── 3. Verify requester is admin ─────────────────────────────────
    ...
    // ── 5. Skip if role is already the same ──────────────────────────
    ...
    // ── 6. Update role via service_role (bypasses RLS) ───────────────
    ...
    console.log(`[update-role] ✅ ${targetProfile.email} role changed from ${targetProfile.role} → ${new_role} by requester ${requester_id}`);
